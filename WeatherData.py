...
>>> help(wd) # prints every docstring in WeatherData
...
def sqlWeather(fname, type, cnn=None):
    ...
    if type == 'airport':
        ...
        while x:
            ...
            VALUES
            ('%s', '%s', %s, %s, %s, %s, %s, %s, %s, %s, %s, '%s', '%s')""" % (type, str(date.date()), str(date.time()),
                                                                                temp, dewpnt, humid, baro, vis,
                                                                                winddir, windspd, windgst, precip,
                                                                                events,cond)
...
def parseASOS(fname, RT=55, IZ=True, ret=False):
    """ DEPRECATED -- USE wd.swmmASOS INSTEAD
    t,p = asos.parseASOS(file_name [, reset_time, include_zeros])
    This function takes a file generated by asos.getASOS
    and parses it into an 2 input files for SWMM (hourly and 5-min data).
    ...
    +-----------------------------------+
    """
    print("This function is deprecated. Using WeatherData.swmmASOS instead")
    swmmASOS(fname, RT=RT, IZ=IZ, ret=ret)
    # f = open(fname, 'r')    # read file
    # X = f.readlines()       # put each line in a list
    # f.close()               # close the file
    # ex = []                 # exception list
    
    # # output array [yyyy, mm, dd, hh, mm, ss, precip]
    # t = np.zeros([len(X), 8], dtype='int')
    

    # for i in range(len(X)):             # go through all of X        
        # X[i] = X[i].split('\n')         # get rid of the line breaks
        # X[i] = X[i][0]                  # ...and the junk afterwards
        
        # # date and time
        # yr = int(X[i][13:17])   # year
        # mo = int(X[i][17:19])   # month
        # da = int(X[i][19:21])   # day
        # hr = int(X[i][37:39])   # hour
        # mi = int(X[i][40:42])   # minute
        
        

        # # throw all that into the output array
        # t[i,0:5] = [yr, mo, da, hr, mi]
        
        # # finding precip values
        # cnt=0
        # for j in range(45,len(X[i])):   # go through each character
            
            # # find precip flags
            # if X[i][j-1:j+2] == ' P0' or X[i][j-1:j+2] == '+P0':  
                
                # # one weird exception
                # try:
                    # t[i,5] = int(X[i][j+1:j+5])
                # except:
                    # t[i,5] = 0
                    # print 'weird exception at line ' + str(i)
                    # ex.append(X[i])
                        
    # #p = zeros([len(t),1], dtype='float')    # create output array for precip
    # f1 = open(fname[0:4]+'_5min.dat', 'w')
    
    # # go through all of the precip data and take out 
    # # accumulated rain and convert bucket tips to inches
    # for i in range(1,len(t)):

            
        # if (t[i-1,5] <= t[i,5]) and (t[i,4] != RT):
            # t[i,6] = (t[i,5] - t[i-1,5])
        ...
        # L1 = []
        # L1.append('%s\t' % (fname[0:4],))
        
        # # set up string to write to file by
        # # looping through all time elements
        # for j in range(5):
            # L1.append('%02d\t' % (t[i,j],))  # add each time element
                        
        
            
        # # excluding zero rain values:
        # if IZ == False:
            
            # #5 min data
            # if t[i,6] > 0:
                # p5 = float(t[i,6])/100.         # convert precip value
                # L1.append('%0.2f\n' % (p5,))    # add precip value
                # f1.writelines(L1)               # write to output file

            
        # # including zero rain values:
        # elif IZ == True:
            # p5 = float(t[i,6])/100.
            # L1.append('%0.2f\n' % (p5,))               # add precip value
            # f1.writelines(L1)                       # write to output file
    ...
    # p = array(t[:,-1], dtype='float')
    # p = p/100.

    # if ret:
        # return t,p,ex          # output the variables
        
def stationRead(STA, start_date, end_date):
    """DEPRECATED -- USE wd.getWeather INSTEAD
    Download data from wunderground weather stations 
    Use: 
    >>> import weatherData as WD
    >>> WD.stationRead([STA_ID], [start_date], [end_date])
    where STA_ID is a string of the station identification number 
    of the wunderground weather station and [*_date] is an array
    example: 
    >>> WD.stationRead('KORPORTL51', [2008, 12, 01], [2008, 12, 31])
    will download all of the data from station KORPORTL51 for the 
    month of December in 2008.
    +-----------------------------------+
    | Paul M. Hobson             __o    |
    ...
    +-----------------------------------+
    """
    print("This function is deprecated. Please use WeatherData.getWeather instead")
    # split up dates
    y1, m1, d1 = start_date
    y2, m2, d2 = end_date
    ...
    for n in range(SD,ED+1):
        ...
        if n == SD:
            fileGetter(STA, [Y,M,D], 'PWS', f, header=True)
        else:
            fileGetter(STA, [Y,M,D], 'PWS', f)
            

    f.close()                           # close the file.  
    print 'File saved in ' + fname      # time to go home. we're done here.

def airportRead(APC, start_date, end_date):
    """DEPRECATED -- USE wd.getWeather INSTEAD
    Download data from wunderground airports
    Use: 
    >>> import weatherData as WD
    >>> WD.airportRead([STA_ID], [start_date], [end_date])
    where STA_ID is a string of the station identification number 
    of the wunderground weather station and [*_date] is an array
    example: 
    >>> WD.airportRead('KPDX', [2008, 12, 01], [2008, 12, 31])
    will download all of the data from PDX ariport for the 
    month of December in 2008.
    +-----------------------------------+
    | Paul M. Hobson             __o    |
    | phobson@geosyntec.com    _`\<,_   | 
    ...
    +-----------------------------------+
    """
    print("This function is deprecated. Use WeatherData.getWeather instead")
    # split up dates
    y1, m1, d1 = start_date
    y2, m2, d2 = end_date
    ...
    fname = '%s_%d%02d%02dto%d%02d%02d.csv' % (APC, y1, m1, d1, y2, m2, d2)
    f = open(fname, 'w')
    ...
    for n in range(SD,ED+1):
        ...
        if n == SD:
            fileGetter(APC, [Y,M,D], 'airport', f, header=True)
        else:
            fileGetter(APC, [Y,M,D], 'airport', f)  
        

    f.close()                       # close the file.  time to go home, we're done here
